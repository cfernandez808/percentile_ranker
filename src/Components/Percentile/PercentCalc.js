...
export default class Calc {
  constructor(user, type, companies, scores) {
    this.user = user
  ...
  userCompanyScore () {
    return this.companies.filter(company => company.company_id === this.userCompId)[0].fractal_index
  }

  //Finds all similar companies to the Candidate's company
  similarCompanies () {
    return this.companies.filter(company => isSimilar(company.fractal_index, this.userCompanyScore()))
  }
  ...
  filterScores () {
    return this.scores.filter(user => user.title === this.userTitle && this.arrOfCompIds().includes(user.company_id))
  }
